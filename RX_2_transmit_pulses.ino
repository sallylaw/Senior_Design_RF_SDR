...
{    
    if ( radio.available() )        // if there is data ready
    {
      ...
      while (!done)
      {
        ...
        /************************/
        //testing with A0 input, need to change value to message later
        sample = map(message, 0, 4095, 0, HzSample);
        analogWrite(6, sample);  // write the selected waveform on DAC0
  
     /*   Serial.write( 0xff );                                                         
        Serial.write( (message >> 8) & 0xff );                                            
        Serial.write( message & 0xff );*/
        
       // analogWrite(analogOutPin, message);
